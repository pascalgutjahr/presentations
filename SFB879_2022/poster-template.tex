...
%	 Introduction
% ----------------------------------------------------------------------------------------
\begin{sectionbox*}{Introduction \& Problem}
  Large scale neutrino telescopes and muon tomography reconstruct 
  incoming muons to estimate the initial direction of muon neutrinos and muons. 
...
\begin{sectionbox*}[tu02]{Methodology}
  The lepton propagator PROPOSAL~\cite{proposal_koehne}, maintained here in Dortmund, is used to 
...
\begin{sectionbox*}[tu02]{Good Agreement with MUSIC and Geant4}
  \begin{multicols*}{2}
    Other common tools to simulate particles through matter are MUSIC~\cite{MUSIC} and Geant4~\cite{Geant4}. Here,
    1 Mio. muons are propagated with energies of 2\,TeV through water over a distance of 3\,km. PROPOSAL provides deflection parametrizations of Geant4 and 
    Van Ginneken (v.G.) and Molière and Highland scattering are studied.
    \begin{itemize}
      \item Good agreement in angular deflection 
      \item Small deviations in lateral displacement $x$ 
      \item Geant4 and PROPOSAL using Molière scattering lead to largest displacements
...
%  Colors 1)
% ----------------------------------------------------------------------------------------
\begin{sectionbox*}[tu02]{Good Data-MC Agreements}
  ...
  \begin{multicols*}{2}
    \raggedright
    \includegraphics[width=0.44\textwidth]{img/attwood_comparison_moliere_199MeV_final_multi_mean_deg.pdf}
    \columnbreak
    \raggedleft
    \includegraphics[width=0.44\textwidth]{img/akimenko_comparison_moliere_E7301MeV_final_multi_mean_deg.pdf}
  \end{multicols*}
\end{sectionbox*}
...
\begin{sectionbox*}{Results}
  \begin{multicols*}{2}
    ...
    \columnbreak
    \FloatBarrier
    Muons are propagated through ice with several initial energies to different final energies. The lower the final energy, the larger the muon deflection. Since the medians overlap for different initial energies, the total muon deflection is 
    nearly independent of the initial energy. Thus the muon energy reconstructed at the detector entry can be used to estimate the muon deflection before the entry. 
    
    All medians are fitted by a polynomial which can be used to estimate the muon deflection as a function of the final muon energy. The function can be found in Ref.~\cite{muon_deflection}.
...
\begin{sectionbox}{Conclusion}
  ...
  tools MUSIC and Geant4 and two measurements. 
...
\begin{sectionbox}[tu03]{Submission}
  All results presented on this poster are submitted to the \textit{European Physics Journal C (EPJ C)} and they are already available 
  on ArXiv under Ref.~\cite{muon_deflection}. 
\end{sectionbox}

% \begin{sectionbox}[tu03]{PROPOSAL}
\begin{infobox}  
  There is another PROPOSAL poster presented by Jean-Marco Alameddine at the REAML Summer School 2022.
  You can also checkout our git repository 
